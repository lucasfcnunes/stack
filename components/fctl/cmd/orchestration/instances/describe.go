...
func NewDescribeCommand() *cobra.Command {
	return fctl.NewCommand("describe <instance-id>",
		...
		fctl.WithRunE(func(cmd *cobra.Command, args []string) error {
			...
			ret, err := client.Orchestration.GetInstanceHistory(cmd.Context(), operations.GetInstanceHistoryRequest{
				InstanceID: args[0],
			})
			if err != nil {
				return err
			...
			for i, history := range ret.GetWorkflowInstanceHistoryResponse.Data {
